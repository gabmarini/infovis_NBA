	...
	players.draw = function(college, score_type){
		d3.json("http://infovis-nba.herokuapp.com/players/college/"+ college, function(data){
			...
			var svg = d3.select("#bar-college"),
				margin = {top: 15, right: 15, bottom: 15, left: 130},
			...
			x.domain([0, d3.max(player, function(d) { return d[score_type]; })]);
	    	y.domain(player.map(function(d) { return d.name; })).padding(0.1);
